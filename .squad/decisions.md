...
### D-007: Multi-PDF Batch Processing — Flow 06 (Manual Batch Trigger)
...
Batch processing approach chosen over scheduled job for MVP:
- **Why manual trigger**: immediate feedback, better for testing and demo, no async delays
- **Why not scheduled (yet)**: saves complexity, can add Phase 3 if high-volume needs arise
- **Flow 06 design**: List files in FormIntake → loop through → call Flow 01 per file → 5 sec delay between calls
- **Input modification to Flow 01**: Add optional `OptionalFileId` parameter; if provided, use that file; otherwise use trigger context
- **No deduplication (MVP scope)**: First pass doesn't prevent reprocessing; Phase 2 enhancement adds file-ID tracking

Implementation guide: `Flows/MVP-06-BATCH-PROCESSOR-SETUP.md`. Can be upgraded to scheduled + parallel in Phase 3 without breaking Flow 01/02/03.
